...
- **Combo — F+C Thrust × the scarcity book** — the timing overlay applied to **your** target-weighted basket (the alpha-from-scarcities selection) vs buy-&-hold the book, each checked against −35%. **Honest caveat:** the book's own history is short and (so far) bull-only, so this line can't show the timing's tail benefit until the book lives through a real drawdown.
- **Combo through real bears** — because the book is too young, the build-out sleeve is mapped to **long-history liquid analogues** (SMH/XLI/XLU/XME/ITA/PHO/XLK) and the same F+C Thrust ladder is run through **2008/2020/2022**. This is where the *combo* (theme × timing) is actually cycle-tested. It's an **analogue** — composition/weights approximate, mapping chosen with hindsight, survivors — so read the **drawdown-cut and Calmar delta**, not the absolute CAGR. See `docs/DRAWDOWN-DEFENSE.md` → *Backtest realism* for the full caveats (tax not modeled = IRA-only; execution optimism; survivor/hindsight bias).
