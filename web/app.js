...
function renderMetrics() {
  ...
  const scCombo = fcb ? `<p class="foot"><strong>Combo — F+C Thrust × the scarcity book</strong> (the timing overlay on <em>your</em> target-weighted basket, ${fcb.years}y ${esc(fcb.window || "")}): buy-&amp;-hold CAGR ${(fcb.buyhold.cagr * 100).toFixed(0)}% / maxDD −${(fcb.buyhold.max_drawdown * 100).toFixed(0)}% ${mand(fcb.breach_35?.buyhold)} → <strong>timed</strong> CAGR ${(fcb.fc_thrust.cagr * 100).toFixed(0)}% / maxDD −${(fcb.fc_thrust.max_drawdown * 100).toFixed(0)}% ${mand(fcb.breach_35?.fc_thrust)} (Calmar ${num(fcb.buyhold.calmar)}→${num(fcb.fc_thrust.calmar)}). <span class="foot">⚠ short, bull-only window — the timing edge is only visible through a real drawdown (see below); selection edge is graded by the Track record + signal backtest.</span></p>` : "";
  ...
  const scComboCycle = fcx ? `<p class="foot"><strong>Combo through real bears</strong> (build-out sleeve → long-history <em>analogues</em>, ${fcx.years}y ${esc(fcx.window || "")}): buy-&amp;-hold maxDD −${(fcx.buyhold.max_drawdown * 100).toFixed(0)}% ${mand(fcx.breach_35?.buyhold)} → <strong>F+C Thrust</strong> maxDD −${(fcx.fc_thrust.max_drawdown * 100).toFixed(0)}% ${mand(fcx.breach_35?.fc_thrust)}, CAGR ${(fcx.buyhold.cagr * 100).toFixed(0)}%→${(fcx.fc_thrust.cagr * 100).toFixed(0)}%, Calmar ${num(fcx.buyhold.calmar)}→${num(fcx.fc_thrust.calmar)}${(fcx.episodes || []).length ? ` — crashes: ${fcx.episodes.map((e) => `${String(e.from || "").slice(0, 4)} −${(e.buyhold_dd * 100).toFixed(0)}%→−${(e.fc_dd * 100).toFixed(0)}%${e.helped ? "" : "⚠"}`).join(", ")}` : ""}.<br><span class="foot">⚠ ANALOGUE basket (${esc((fcx.basis || []).join(", "))}) standing in for the younger holdings — composition/weights approximate, mapping chosen with hindsight, survivors. Tests the theme×timing methodology through a cycle, NOT the exact book.</span></p>` : "";
